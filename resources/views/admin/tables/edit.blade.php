...
@section('content')
    @include('layouts.components.admin.contentheader', ['title' => __('edit').' '.__('table'),'breadcrumbs'=>[__('tables')=>'/admin/tables',__('edit')=>'#']])
    <section class="content bcg-white">
        <form role="form" method="POST" action="{{route('tables.update',$table->id)}}">
            <div class="box-body">
                <div class="form-group">
                    <label for="key" class="capitalize">{{__('key')}}</label>
                    <input type="text" class="form-control" id="key" required
                           value="{{ $table->key }}" name="key">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="institution_id" name="institution_id">
                        <option value="NULL">{{__('institution is not assigned')}}</option>
                        @foreach($institutions as $institution)
                            <option value="{{$institution->id}}" @if($table->institution_id == $institution->id) selected @endif>{{$institution->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
