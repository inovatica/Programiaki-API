...
@section('content')
    @include('layouts.components.admin.contentheader', ['title' => __('remove').' '.__('level'),'breadcrumbs'=>[__('levels') => route('levels.index'),__('remove')=>'#']])
    <section class="content bcg-white">
        <form role="form" method="POST" action="{{route('levels.destroy',$level->id)}}">
            <div class="box-body">
                <h2 class="capitalize center-align">{{ __('are_you_sure_to_remove') }}</h2>
                <h2 class="capitalize center-align">"{{ $level->name }}" z gry "{{ $level->game->name }}"?</h2>
            </div>
