...
@section('content')
    @include('layouts.components.admin.contentheader', ['title' => __('remove').' '.__('avatar'),'breadcrumbs'=>[__('avatars') => route('avatars.index'),__('remove')=>'#']])
    <section class="content bcg-white">
        <form role="form" method="POST" action="{{route('avatars.destroy',$avatar->id)}}">
            <div class="box-body">
                <h2 class="capitalize center-align">{{ __('are_you_sure_to_remove') }}</h2>
                <h2 class="capitalize center-align">"{{ $avatar->name }}"?</h2>
            </div>
